...
for t in intake initiation dispositions sentencing diversion; do
  ...
  curl -L --retry 5 --retry-delay 15 -sS -A "Mozilla/5.0 (datapond-maintenance)" -o "$f.part" \
    "https://datacatalog.cookcountyil.gov/api/views/$id/rows.csv?accessType=DOWNLOAD" \
    && mv "$f.part" "$f" && echo "ok $f $(stat -c %s "$f") bytes $(wc -l < "$f") lines" || { rm -f "$f.part"; echo "FAIL $f"; }
done
